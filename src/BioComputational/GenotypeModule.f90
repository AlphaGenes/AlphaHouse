...
module GenotypeModule
	...
	contains
		...
		procedure :: newGenotypeInt
		procedure :: newGenotypeHap
		procedure :: newGenotypeMissing
		generic :: Genotype => newGenotypeHap, newGenotypeInt,newGenotypeMissing
	...
	contains
		...
		!---------------------------------------------------------------------------
		subroutine newGenotypeInt(g,geno, lock)
		...
		!> @return	New genotype object
		!---------------------------------------------------------------------------
		subroutine newGenotypeHap(g, h1, h2, lock)
			use HaplotypeModule
		...
		!---------------------------------------------------------------------------
		subroutine newGenotypeMissing(g, length)
			...
			g%hasLock = .false.
		end subroutine newGenotypeMissing
		...
		function complement(g,h) result(c)
			...
			call c%Haplotype(phase,missing,g%length)
			deallocate(phase)
			deallocate(missing)
		...
		subroutine setFromHaplotypesIfMissing(g,h1,h2)
			...
			call newGenotypeHap(gFromH,h1,h2)
